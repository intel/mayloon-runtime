...
public class SpannableStringBuilder implements CharSequence, Editable {
	...
	public SpannableStringBuilder(CharSequence text) {
		if(text != null){
			SpannableStringBuilderInit(text, 0, text.toString().length());
		}
		else{
	...
	private void SpannableStringBuilderInit(CharSequence text, int start, int end){
		if(text == null){
			throw new NullPointerException("the first parameter is null");
		}
	...
	public SpannableStringBuilder(CharSequence text, int start, int end) {
		SpannableStringBuilderInit(text,start,end);
	}

	public static SpannableStringBuilder valueOf(CharSequence source) {
       /**@Mayloon workround
        * @j2sNative
        * if(source == null || source.valueOf() == null){
        *     throw new NullPointerException("the first parameter is null");
		* }
        */{}
	...
	private void resizeFor(int size) {
		...
		System.arraycopy(mText, mText.length - after, newtext, newlen - after,
				after);
	...
	private void moveGapTo(int where) {
		...
		if (where < mGapStart) {
			...
			System.arraycopy(mText, where, mText, mGapStart + mGapLength
					- overlap, overlap);
		} else /* where > mGapStart */{
			int overlap = where - mGapStart;

			System.arraycopy(mText, where + mGapLength - overlap, mText,
					mGapStart, overlap);
		}
	...
	/*public SpannableStringBuilder insert(int where, CharSequence tb, int start, int end) {
		//System.out.println("SpannableStringBuilder insert2 " + tb.toString());
	    return replace(where, where, tb, start, end);
	}*/

	// Documentation from interface
	public SpannableStringBuilder insert(int where, CharSequence tb) {
		//System.out.println("SpannableStringBuilder insert " + tb.toString());
		return replace(where, where, tb, 0, tb.toString().length());
	}
	...
	public SpannableStringBuilder append(CharSequence text) {
		...
		return replace(length, length, text, 0, text.toString().length());
	}
	...
	public SpannableStringBuilder append(CharSequence text, int start, int end) {
       /**@Mayloon workround
        * @j2sNative
        * if(text == null || text.valueOf() == null){
        * throw new NullPointerException("the first parameter is null");
        * }
        */{}
        int length = length();
	...
	private int change(int start, int end, CharSequence tb, int tbstart,
			int tbend) {
		return change(true, start, end, tb, tbstart, tbend);
	}

	private int change(boolean notify, int start, int end, CharSequence tb,
		...
		if (tbend - tbstart >= mGapLength + (end - start))
			resizeFor(mText.length - mGapLength + tbend - tbstart
					- (end - start));
		...
		if (tb instanceof Spanned) {
			...
			for (int i = 0; i < spans.length; i++) {
				...
				if (getSpanStart(spans[i]) < 0) {
					setSpan(false, spans[i], st - tbstart + start, en - tbstart
							+ start, sp.getSpanFlags(spans[i]));
				}
			}
		}

		//System.out.println("tbend tbStart end start " + tbend + tbstart + end + start);
		...
		for (int i = mSpanCount - 1; i >= 0; i--) {
			...
			if (mSpanEnds[i] < mSpanStarts[i]) {
				...
				System.arraycopy(mSpanStarts, i + 1, mSpanStarts, i, mSpanCount
						- (i + 1));
				System.arraycopy(mSpanEnds, i + 1, mSpanEnds, i, mSpanCount
						- (i + 1));
				System.arraycopy(mSpanFlags, i + 1, mSpanFlags, i, mSpanCount
						- (i + 1));
		...
		//System.out.println("changed " + notify);
		if (notify) {
			sendTextChange(recipients, start, end - start, tbend - tbstart);
			sendTextHasChanged(recipients);
		}
	...
	public SpannableStringBuilder replace(final int start, final int end,
		...
		//    	//System.out.println("SpannableStringBuilder replace " + tb.toString());
		if (end == start && tbstart == tbend) {
			//        	//System.out.println("SpannableStringBuilder replace1");
			return this;
		}

		//    	//System.out.println("SpannableStringBuilder replace2");

		if (end == start || tbstart == tbend) {
			//        	//System.out.println("SpannableStringBuilder replace3");
			change(start, end, tb, tbstart, tbend);
		} else {
			//        	//System.out.println("SpannableStringBuilder replace4");
			...
			int inserted = change(false, start + 1, start + 1, tb, tbstart,
					tbend);
	...
	private void setSpan(boolean send, Object what, int start, int end,
			int flags) {
		int nstart = start;
		int nend = end;
		...
		if ((null == mSpans) || (mSpanCount + 1 >= mSpans.length)) {
	...
	public int getSpanStart(Object what) {
		//    	//System.out.println("getSpanStart " + what);
		int count = mSpanCount;
		Object[] spans = mSpans;

		for (int i = count - 1; i >= 0; i--) {

			//        	//System.out.println("getSpanStart span i" + spans[i]);
			if (spans[i] == what) {
	...
	public <T> T[] getSpans(int queryStart, int queryEnd, Class<T> kind) {
		...
		//System.out.println("kind is " + kind.getName());
	...
	public void getChars(int start, int end, char[] dest, int destoff) {
		...
		if (end <= mGapStart) {
			...
			System.arraycopy(mText, start + mGapLength, dest, destoff, end
					- start);
		} else {
			System.arraycopy(mText, start, dest, destoff, mGapStart - start);
			System.arraycopy(mText, mGapStart + mGapLength, dest, destoff
					+ (mGapStart - start), end - mGapStart);
		}
	}
	...
	private TextWatcher[] sendTextWillChange(int start, int before, int after) {
		...
		//System.out.println("sendTextWillChange " + recip.length);

		for (int i = 0; i < n; i++) {
			//System.out.println("sendTextWillChange " + recip[i].getClass().getName());
	...
	private void sendTextChange(TextWatcher[] recip, int start, int before,
			int after) {
		//System.out.println("sendTextChange");
		int n = recip.length;
	...
	private void sendTextHasChanged(TextWatcher[] recip) {
		...
		for (int i = 0; i < n; i++) {
			if ((recip[i].getClass().getName()
					.equals("android.text.Selection.START"))
					|| (recip[i].getClass().getName()
							.equals("android.text.Selection.END"))) {
				continue;
			}
