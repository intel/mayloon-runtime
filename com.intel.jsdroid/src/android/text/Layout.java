...
public abstract class Layout {
    ...
    /* package */ static final EmojiFactory EMOJI_FACTORY = null;
    ...
    public static float getDesiredWidth(CharSequence source, int start, int end, TextPaint paint) {
        ...
        for (int i = start; i <= end; i = next) {
             ...
             * @j2sNative
             * if (source.isHtml) {
             *     next = android.text.TextUtils.indexOf(source, "<br />", i, end);
             * } else {
...
//            float w = measureText(paint, workPaint, source, i, next, null, true, null);
            float w = Styled.measureText(paint, workPaint, source, i, next, null);
        ...
        for (int i = 0; i < dirs.mDirections.length; i++) {
            ...
            if (start <= there && end >= here) {
                ...
                if (st != en) {
                    ...
                    dest.addRect(h1, top, h2, bottom, Direction.CW);
                }
            }
        ...
        if (startline == endline) {
            ...
            if (getParagraphDirection(startline) == DIR_RIGHT_TO_LEFT)
                dest.addRect(getLineLeft(startline), top,
                              0, getLineBottom(startline), Direction.CW);
            else
                dest.addRect(getLineRight(startline), top,
                              width, getLineBottom(startline), Direction.CW);

            for (int i = startline + 1; i < endline; i++) {
                top = getLineTop(i);
                bottom = getLineBottom(i);
                dest.addRect(0, top, width, bottom, Direction.CW);
            }
            ...
            if (getParagraphDirection(endline) == DIR_RIGHT_TO_LEFT)
                dest.addRect(width, top, getLineRight(endline), bottom, Direction.CW);
            else
                dest.addRect(0, top, getLineLeft(endline), bottom, Direction.CW);
        }
    }
