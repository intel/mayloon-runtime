...
public class TimeUtils {
    ...
    /** @hide Just for debugging; not internationalized. */
    public static void formatDuration(long duration, int fieldLen) {
        synchronized (sFormatSync) {
            int len = formatDurationLocked(duration, fieldLen);
            System.out.println(new String(sFormatStr, 0, len));
        }
    }
    ...
    /** @hide Just for debugging; not internationalized. */
    public static void formatDuration(long time, long now) {
        if (time == 0) {
            System.out.println("--");
            return;
        }
        formatDuration(time-now, 0);
    }
}
