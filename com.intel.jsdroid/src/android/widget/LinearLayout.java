...
public class LinearLayout extends ViewGroup {
	...
	public LinearLayout(Context context, AttributeSet attrs) {
		...
		int index = a.getInt(
				com.android.internal.R.styleable.LinearLayout_orientation, 0);
		setOrientation(index);

		index = a.getInt(com.android.internal.R.styleable.LinearLayout_gravity,
				-1);
		if (index >= 0) {
			// System.out.println("setGravity: " + index);
			setGravity(index);
		}
		...
		mBaselineAlignedChildIndex = a
				.getInt(com.android.internal.R.styleable.LinearLayout_baselineAlignedChildIndex,
						-1);

		// TODO: Better name, add Java APIs, make it public
		mUseLargestChild = a.getBoolean(
				R.styleable.LinearLayout_useLargestChild, false);

		a.recycle();
	}

    /** @Mayloon
     *  For TabWidget three-parameters constructor can be used in JS
     */
	...
	void measureVertical(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// System.out.println("mBaselineAlignedChildIndex: "
		// + mBaselineAlignedChildIndex + ", mBaselineAligned: "
		// + mBaselineAligned + ", mUseLargestChild: " + mUseLargestChild);
		...
		for (int i = 0; i < count; ++i) {
			...
			if (heightMode == MeasureSpec.EXACTLY && lp.height == 0
				...
				measureChildBeforeLayout(child, i, widthMeasureSpec, 0,
						heightMeasureSpec, totalWeight == 0 ? mTotalLength : 0);
				...
				//				System.out.println("Child ID: " + child.getUIElementID()
				//						+ ", measured height: " + childHeight);
				final int totalLength = mTotalLength;
				...
				if (useLargestChild) {
					largestChildHeight = Math.max(childHeight,
							largestChildHeight);
				}
			}
		...
		if (useLargestChild && heightMode == MeasureSpec.AT_MOST) {
			...
			for (int i = 0; i < count; ++i) {
				...
				mTotalLength = Math.max(totalLength, totalLength
						+ largestChildHeight + lp.topMargin + lp.bottomMargin
						+ getNextLocationOffset(child));
			}
		}
		...
		if (delta != 0 && totalWeight > 0.0f) {
			...
			for (int i = 0; i < count; ++i) {
				...
				mTotalLength = Math.max(totalLength,
						totalLength + child.getMeasuredHeight() + lp.topMargin
								+ lp.bottomMargin
								+ getNextLocationOffset(child));
			}
			...
			// Add in our padding
			mTotalLength += mPaddingTop + mPaddingBottom;
			// TODO: Should we recompute the heightSpec based on the new total
			// length?
		} else {
			alternativeMaxWidth = Math.max(alternativeMaxWidth,
					weightedMaxWidth);
		...
		//		System.out.println("LL ID: " + this.getUIElementID() + ", width: "
		//				+ resolveSize(maxWidth, widthMeasureSpec) + ", height: "
		//				+ heightSize);
	...
	void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (widthMode == MeasureSpec.EXACTLY && lp.width == 0
					&& lp.weight > 0) {
				// Optimization: don't bother measuring children who are going
				// to use
				// leftover space. These views will get measured again down
				...
				if (baselineAligned) {
					final int freeSpec = MeasureSpec.makeMeasureSpec(0,
				...
				measureChildBeforeLayout(child, i, widthMeasureSpec,
						totalWeight == 0 ? mTotalLength : 0, heightMeasureSpec,
						0);
			...
			if (baselineAligned) {
				...
				if (childBaseline != -1) {
					...
					maxAscent[index] = Math
							.max(maxAscent[index], childBaseline);
					maxDescent[index] = Math.max(maxDescent[index], childHeight
							- childBaseline);
				}
			}
		...
		if (useLargestChild && widthMode == MeasureSpec.AT_MOST) {
			...
			for (int i = 0; i < count; ++i) {
				...
				final LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) child
						.getLayoutParams();
				if (isExactly) {
					mTotalLength += largestChildWidth + lp.leftMargin
							+ lp.rightMargin + getNextLocationOffset(child);
				} else {
					final int totalLength = mTotalLength;
					mTotalLength = Math.max(totalLength, totalLength
							+ largestChildWidth + lp.leftMargin
							+ lp.rightMargin + getNextLocationOffset(child));
				}
			}
		}
		...
		if (delta != 0 && totalWeight > 0.0f) {
			...
			for (int i = 0; i < count; ++i) {
				...
				if (baselineAligned) {
					...
					if (childBaseline != -1) {
						// Translates the child's vertical gravity into an index
						// in the range 0..2
						final int gravity = (lp.gravity < 0 ? mGravity
								: lp.gravity) & Gravity.VERTICAL_GRAVITY_MASK;
						final int index = ((gravity >> Gravity.AXIS_Y_SHIFT) & ~Gravity.AXIS_SPECIFIED) >> 1;

						maxAscent[index] = Math.max(maxAscent[index],
								childBaseline);
			...
			if (maxAscent[INDEX_TOP] != -1
				...
				final int ascent = Math
						.max(maxAscent[INDEX_FILL], Math.max(
								maxAscent[INDEX_CENTER_VERTICAL], Math.max(
										maxAscent[INDEX_TOP],
										maxAscent[INDEX_BOTTOM])));
				final int descent = Math.max(maxDescent[INDEX_FILL], Math.max(
						maxDescent[INDEX_CENTER_VERTICAL], Math
								.max(maxDescent[INDEX_TOP],
		...
		//		System.out.println("LL ID: " + this.getUIElementID() + ", width: "
		//				+ widthSize + ", height: "
		//				+ resolveSize(maxHeight, heightMeasureSpec) + ", maxHeight: "
		//				+ maxHeight);
		...
		for (int i = 0; i < count; i++) {
			...
			if (child == null) {
				...
				case Gravity.CENTER_HORIZONTAL:
					// System.out.println("V: Gravity.CENTER_HORIZONTAL");
				...
				setChildFrame(child, childLeft, childTop
						+ getLocationOffset(child), childWidth, childHeight);
	...
	void layoutHorizontal() {
		...
		if (majorGravity != Gravity.LEFT) {
			...
			case Gravity.CENTER_HORIZONTAL:
				// System.out.println("H: Gravity.CENTER_HORIZONTAL");
		...
		for (int i = 0; i < count; i++) {
			...
			if (child == null) {
				...
				setChildFrame(child, childLeft + getLocationOffset(child),
	...
	public static class LayoutParams extends ViewGroup.MarginLayoutParams {
		...
		public LayoutParams(Context c, AttributeSet attrs) {
			...
			//			weight = a
			//					.getInt(com.android.internal.R.styleable.LinearLayout_Layout_layout_weight,
			//							0);
			gravity = a
					.getInt(com.android.internal.R.styleable.LinearLayout_Layout_layout_gravity,
							-1);
			// System.out.println("resolve gravity: " + gravity);
		...
		public String debug(String output) {
			return output + "LinearLayout.LayoutParams={width="
