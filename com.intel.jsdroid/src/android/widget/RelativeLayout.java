...
public class RelativeLayout extends ViewGroup {
    ...
    private void initFromAttributes(Context context, AttributeSet attrs) {
    	...
    	TypedArray a = context.obtainStyledAttributes(attrs, 
    			com.android.internal.R.styleable.RelativeLayout);
        mIgnoreGravity = a.getResourceId(com.android.internal.R.styleable.RelativeLayout_ignoreGravity, View.NO_ID);
        mGravity = a.getInt(com.android.internal.R.styleable.RelativeLayout_gravity, mGravity);
        a.recycle();
    }
    ...
    public void setGravity(int gravity) {
        Log.d(LOG_TAG, "set gravity to: " + gravity);
        if (mGravity != gravity) {
    ...
    public void setHorizontalGravity(int horizontalGravity) {
        Log.d(LOG_TAG, "set horizontal gravity to: " + horizontalGravity);
    ...
    public void setVerticalGravity(int verticalGravity) {
        Log.d(LOG_TAG, "set vertical gravity to: " + verticalGravity);
    ...
    private void sortChildren() {
        ...
        if (DEBUG_GRAPH) {
            Log.d(LOG_TAG, "=== Sorted vertical children");
            graph.log(getResources(), ABOVE, BELOW, ALIGN_BASELINE, ALIGN_TOP, ALIGN_BOTTOM);
            Log.d(LOG_TAG, "=== Sorted horizontal children");
        ...
        if (DEBUG_GRAPH) {
            Log.d(LOG_TAG, "=== Ordered list of vertical children");
            for (View view : mSortedVerticalChildren) {
                DependencyGraph.printViewId(getResources(), view);
            }
            Log.d(LOG_TAG, "=== Ordered list of horizontal children");
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int count = getChildCount();
        Log.d(LOG_TAG, "relativelayout onLayout:"+l+","+t+","+r+","+b);
        for (int i = 0; i < count; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != GONE) {
                RelativeLayout.LayoutParams st =
                        (RelativeLayout.LayoutParams) child.getLayoutParams();
                Log.d(LOG_TAG, "relativelayout child:UIElementID"+ child.getUIElementID()+" "+st.mLeft+","+st.mTop+","+st.mRight+","+st.mBottom);
       ...
       private SparseArray<Node> mKeyNodes = new SparseArray<Node>();
        ...
        static void printViewId(Resources resources, View view) {
            if (view.getId() != View.NO_ID) {
                Log.d(LOG_TAG, resources.getResourceName(view.getId()));
            } else {
                Log.d(LOG_TAG, "NO_ID");
            }
        }
        ...
        private static void printdependents(Resources resources, Node node, StringBuilder buffer) {
            ...
            if (node.dependents.size() == 0) {
            	System.out.println(LOG_TAG+":"+buffer.toString());
            } else {
                for (Node dependent : node.dependents) {
        ...
        static class Node implements Poolable<Node> {
            ...
            //TODO: potential bug exists,li
            private static final Pool<Node> sPool = Pools.finitePool(new PoolableManager<Node>() {
