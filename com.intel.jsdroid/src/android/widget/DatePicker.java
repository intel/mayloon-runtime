...
public class DatePicker extends FrameLayout {
    ...
    public DatePicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        //Formatter is not available for now
...
//        }
    ...
    private void updateDaySpinner() {
        ...
        //Workaround: use created getMaxDays 
        //instead of orinal cal.getActualMaximum
        int max = getMaxDays(cal);
        mDayPicker.setRange(1, max);
        mDayPicker.setCurrent(mDay);
    }

    private int getMaxDays(Calendar cal) {
        int[] daysInMonth = new int[] { 31, 28, 31, 30, 31, 30, 31, 31,
                30, 31, 30, 31 };
        int yearIndex = cal.get(Calendar.YEAR);
        int monthIndex = cal.get(Calendar.MONTH);
        int lastDate = daysInMonth[monthIndex];
        if(monthIndex == 1 && isLeapYear(yearIndex)) lastDate++;
        return lastDate;
    }
