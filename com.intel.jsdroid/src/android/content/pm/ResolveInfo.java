 ...
 * IntentFilter. This partially corresponds to information collected from the
 * AndroidManifest.xml's &lt;intent&gt; tags.
 */
public class ResolveInfo implements Parcelable {
	/**
	 * The activity that corresponds to this resolution match, if this
	 * resolution is for an activity. One and only one of this and serviceInfo
	 * must be non-null.
	 */
	...
	/**
	 * The declared priority of this match. Comes from the "priority" attribute
	 * or, if not set, defaults to 0. Higher values are a higher priority.
	 */
	public int priority;

	/**
	 * Order of result according to the user's preference. If the user has not
	 * set a preference for this result, the value is 0; higher values are a
	 * higher priority.
	 */
	 ...
	 * and {@link IntentFilter#MATCH_ADJUSTMENT_MASK
	 * IntentFiler.MATCH_ADJUSTMENT_MASK}.
	 */
	public int match;

	/**
	 * Only set when returned by
	 * {@link PackageManager#queryIntentActivityOptions}, this tells you which
	 * of the given specific intents this result came from. 0 is the first in
	 * the list, < 0 means it came from the generic Intent query.
	 */
	public int specificIndex = -1;

	/**
	 * This filter has specified the Intent.CATEGORY_DEFAULT, meaning it would
	 * like to be considered a default action that the user can perform on this
	 * data.
	 */
	...
	/**
	 * The actual string retrieve from <var>labelRes</var> or null if none was
	 * provided.
	 */
	 ...
	 *            item.
	 * 
	 * @return Returns a Drawable containing the resolution's icon. If the item
	 *         does not have an icon, the default activity icon is returned.
	 */
	...
	/**
	 * Return the icon resource identifier to use for this match. If the match
	 * defines an icon, that is used; else if the activity defines an icon, that
	 * is used; else, the application icon is used.
	 * 
	...
	public void dump(Printer pw, String prefix) {
		...
		pw.println(prefix + "priority=" + priority + " preferredOrder="
				+ preferredOrder + " match=0x" + Integer.toHexString(match)
				+ " specificIndex=" + specificIndex + " isDefault=" + isDefault);
	...
	public String toString() {
		...
		return "ResolveInfo{"
				...
				+ ci.name + " p=" + priority + " o=" + preferredOrder + " m=0x"
