...
final class StringBlock {
	private ResStringPool mNative = null;
	private boolean mOwnsNative;
    private final boolean mUseSparse;
	private CharSequence[] mStrings = null;
     ...
     */
    public StringBlock(ResStringPool obj, boolean useSparse) {
        mNative = obj;
        mUseSparse = useSparse;
        mOwnsNative = false;
    }

	private static final ResStringPool nativeCreate(byte[] data, int offset,
			int size) {
		ResStringPool sp = null;
		try {
			sp = new ResStringPool(data, offset, size, true);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return sp;
	...
	private static final int nativeGetSize(ResStringPool obj) {
		return obj.size();
	}

	private static final String nativeGetString(ResStringPool obj, int idx) {
		return obj.stringAt(idx);
	}

    private static final int[] nativeGetStyle(ResStringPool obj, int idx) {
        return obj.getStyle(idx);
    }

	private static final int nativeIndexOfString(ResStringPool obj, String str) {
		return obj.indexOfString(str);
	}

	private static final void nativeDestroy(ResStringPool obj) {
		obj = null;
	}
    ...
    private CharSequence applyStyles(String str, int[] style, StyleIDs ids) {
        ...
        while (i < style.length) {
            int type = style[i];
            Log.v("StringBlock.applyStyles", "Applying style span id=" + type
